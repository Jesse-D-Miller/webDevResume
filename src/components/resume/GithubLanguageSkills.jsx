...
function GithubLanguageSkills() {
  ...
  useEffect (() => {
    async function loadStats() {
      ...
      try {
        if (isCacheStale()) {
          const stats = await fetchGithubLanguageStats(GITHUB_USERNAME, GITHUB_TOKEN);
  ...
  return (
    <div className="github-language-skills">
      <h4>GitHub XP by Language</h4>
      <ul>
        {sortedLanguages.map(([language, bytes]) => (
          <li key={language}>
            <h4>{language}: lvl {(bytes.toLocaleString() / 10000).toFixed(0)}</h4>
